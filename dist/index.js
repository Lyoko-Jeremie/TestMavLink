...
const REGISTRY = Object.assign(Object.assign(Object.assign({}, node_mavlink_1.minimal.REGISTRY), node_mavlink_1.common.REGISTRY), node_mavlink_1.ardupilotmega.REGISTRY);
class CustomProtocolTransformToSerialPort extends stream_1.Transform {
    constructor(options) {
        super(options);
    }
...
class CustomProtocolTransformFromSerialPort extends stream_1.Transform {
    constructor(options) {
        super(options);
...
const port = new serialport_1.SerialPort({ path: 'COM4', baudRate: 115200 });
// from SerialPort
const customProtocolReadStream = new stream_1.PassThrough();
port.pipe(new CustomProtocolTransformFromSerialPort()).pipe(customProtocolReadStream);
// to SerialPort
const customProtocolWriteStream = new stream_1.PassThrough();
...
const reader = customProtocolReadStream
    .pipe(new node_mavlink_1.MavLinkPacketSplitter())
    .pipe(new node_mavlink_1.MavLinkPacketParser());
...
port.on('open', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const commandHeartbeat = new node_mavlink_1.minimal.Heartbeat();
    commandHeartbeat.systemStatus = node_mavlink_1.minimal.MavState.STANDBY;
    yield (0, node_mavlink_1.send)(customProtocolWriteStream, commandHeartbeat, new node_mavlink_1.MavLinkProtocolV2());
    console.log('====== commandSystemTime');
    const commandSystemTime = new node_mavlink_1.common.SystemTime();
    commandSystemTime.timeUnixUsec = BigInt(Date.now()) * BigInt(1000);
    commandSystemTime.timeBootMs = process.uptime() * 1000;
    yield (0, node_mavlink_1.send)(customProtocolWriteStream, commandSystemTime, new node_mavlink_1.MavLinkProtocolV2());
    console.log('====== sendEnd');
}));
